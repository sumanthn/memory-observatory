"""
Experiment: Run sessions WITH full memory system

All 4 memory types active: episodic, semantic, procedural, reflective.
This is the "best case" for memory — full learning capability.
"""
...
def run(session_range: tuple[int, int] | None = None) -> dict:
    """Run the full-memory experiment. Returns experiment summary.

    Args:
        session_range: Optional (start, end) inclusive range to filter sessions.
    """
    print("\n" + "=" * 70)
    print("EXPERIMENT: WITH FULL MEMORY")
    print("Enabled stores: episodic, semantic, procedural, reflective")
    if session_range:
        print(f"Session range: {session_range[0]}-{session_range[1]}")
    print("=" * 70)

    db_path = os.path.join("output", "memories", "full_memory.db")
    ...
    for session in scenarios:
        try:
            ...
            snapshot_memory_db(session["id"], "with_memory", db_path)
